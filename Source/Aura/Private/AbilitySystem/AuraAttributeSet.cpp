...
void UAuraAttributeSet::SetEffectProperties(FGameplayEffectModCallbackData const& Data, FEffectProperties& Props) const
{
	...
	if (IsValid(Props.SourceASC) && Props.SourceASC->AbilityActorInfo.IsValid() && Props.SourceASC->AbilityActorInfo->AvatarActor.IsValid())
	{
		...
		{
			if (APawn const* Pawn = Cast<APawn>(Props.SourceAvatarActor))
			{
...
void UAuraAttributeSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
	...
	if (Data.EvaluatedData.Attribute == GetIncomingDamageAttribute())
	{
		...
		if (LocalIncomingDamage > 0.0f)
		{
			...
			{
				const FGameplayTagContainer TagContainer(FAuraGameplayTags::Get().Effects_HitReact);
...
void UAuraAttributeSet::ShowFloatingText(const FEffectProperties& Props, float Damage, bool bBlockedHit, bool bCriticalHit) const
{
	if (Props.SourceCharacter != Props.TargetCharacter)
	{
		...
		else if (APC = Cast<AAuraPlayerController>(Props.TargetController); APC)
		{
			APC->ShowDamageNumber(Damage, Props.TargetCharacter, bBlockedHit, bCriticalHit);
		}
	}
...
void UAuraAttributeSet::OnRep_ArcaneResistance(FGameplayAttributeData const& OldArcaneResistance) const
{
	GAMEPLAYATTRIBUTE_REPNOTIFY(UAuraAttributeSet, ArcaneResistance, OldArcaneResistance);
}

void UAuraAttributeSet::OnRep_FireResistance(FGameplayAttributeData const& OldFireResistance) const
{
	GAMEPLAYATTRIBUTE_REPNOTIFY(UAuraAttributeSet, FireResistance, OldFireResistance);
}

void UAuraAttributeSet::OnRep_LightningResistance(FGameplayAttributeData const& OldLightningResistance) const
{
	GAMEPLAYATTRIBUTE_REPNOTIFY(UAuraAttributeSet, LightningResistance, OldLightningResistance);
}

void UAuraAttributeSet::OnRep_PhysicalResistance(FGameplayAttributeData const& OldPhysicalResistance) const
{
